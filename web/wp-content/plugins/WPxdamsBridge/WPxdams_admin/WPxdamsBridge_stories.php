...
$storyId                   =  WPxdamsBridge_get_archive_id(); // id of the requested story
$active                         = (WPxdamsBridge_get_option('active',$archiveId) == 1) ? 'checked' : '';  //
$deactive                       = (WPxdamsBridge_get_option('active',$archiveId) == 0) ? 'checked' : '';  //
$storiesNumber              = get_option('WPxdamsBridge_stories_number'); // number of archives;
// $storiesNumber              = get_option('WPxdamsBridge_exhibitions_number'); // number of archives;




if (!$storiesNumber) {  $storiesNumber		= 3; };	
$archivesList                   = $archivesConfig ['WPxdamsBridge_archivesList'][0]; //   array contaning only archives

for($i=1;$i<=$storiesNumber;$i++) {

    $separator		= '  -  ';
    if ($i==1) { $separator	= 'You can configure: ';};
    $menuArchives		=  $menuArchives. $separator .
                                    '<a href="admin.php?page=WPxdamsBridge_stories_'.$i.'">Story N. '.$i.'  </a>';
}	

$menuArchives			=  $menuArchives.' - total number of stories <input type="text" style="width:40px;" name="c_storyNumber_WPxdamsBridge"'. '  value="'.$storiesNumber . '"/>';
  

//$count                          = get_option('WPxdamsBridge_exhibitions_fieldsNumber_'.$storyId); // number of slides;
$count                          = get_option('WPxdamsBridge_story_fieldsNumber_'.$storyId); // number of slides;
...
$storyTitle                 = stripslashes(get_option('WPxdamsBridge_storyTitle_'.$storyId));
//$storyTitle          = get_option('WPxdamsBridge_exhibitionTitle_'.$storyId); 
// create table rows (one for each slide)

for($i=1;$i<$count+1;$i++) {	
      
      $toSplit             = get_option('WPxdamsBridge_story_'.$storyId.'_'.$i);
  //        $toSplit             = get_option('WPxdamsBridge_exhibitions_'.$storyId.'_'.$i);
       ...
       $url                 = $temp [3];
       $printPic            = '';
     //  $field               = 'WPxdamsBridge_exhibitions_'.$storyId.'_'.$i; unsed
  ...
  $before2= '
    <div class="wrap">		
        ...
        <h2>Stories, creation and management </h2> '.
         
            '<form id="ak_sharethis" name="WPxdamsBridge_cnf" action="'.get_bloginfo('wpurl').'/wp-admin/index.php" method="post">'.
		     
                ' <p>By this page it\'s possible to create a new story slider or to manage the existing ones </p> '.
		'<h4> You are managing the story number  '.  	 $storyId . '</h4><br>'.	
			$menuArchives.'<br> 
                <fieldset class="options">
                
                    <input type="hidden" id="c_storyID_WPxdamsBridge" name="c_storyID_WPxdamsBridge" value="'. $storyId .'"><br> '.
                    'Title  
                    <input type="text" style="width:824px" name="c_storyTitle_WPxdamsBridge"'. '  value="'.$storyTitle. '"/><br><br>    
                    <h4>Slides</h4>
                            Number of Slides '.
