	...
	foreach ( $zp_request_queue as $zp_request_account )
	{
		if ( count($zp_request_account["requests"]) > 1 )
		{
			foreach ( $zp_request_account["requests"] as $zp_request_url )
			{
				...
				if ( $checkcache )
				{
					$zp_checkcache = $zp_import_contents->get_request_cache( $zp_request_url, $zpr["update"], $zpr["showtags"] );
		...
		else // Just one request
		{
			...
			if ( $checkcache )
			{
				$zp_checkcache = $zp_import_contents->get_request_cache( $zp_request_account["requests"][0], $zpr["update"], $zpr["showtags"] );
			...
			else
			{
				...
				// Add to existing all-requests json
				else $zp_request["json"] = rtrim($zp_request["json"], "]") . "," . ltrim($zp_imported["json"], "[") . "]";
			}
		}
	} // Request the data
