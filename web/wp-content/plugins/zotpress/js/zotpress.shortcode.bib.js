jQuery(document).ready(function()
{
		...
		jQuery.ajax(
		{
			...
			success: function(data)
			{
				...
				if ( zp_items.status == 'empty' )
				{
					...
					console.log( 'There was a Zotpress error: ' + zp_items.data );
					...
					jQuery( "#"+zp_items.instance+" .zp-List" )
						.removeClass( 'loading' )
						.append( '<p'+hideErrMsg+'>There was a Zotpress error: '+zp_items.data+'</p>' );
				}
	...
	function zp_bib_reformat( $instance, zp_items, zp_params )
	{
		...
		if ( zp_params.zpTitle !== false )
		{
			...
			// Then, sort title sort, depending on type
			...
			titleSortedEntriesOrder.forEach( function(orderedType, i)
			{
				...
				{
					if ( ( zp_items.meta.request_last != 0
								&& zp_items.meta.request_next != 0 )
							&& jQuery("#"+$instance.attr("id")+" .zp-List h3[rel='"+orderedType+"']").length == 0 )
					{
						// Then, place new header before/after, based on sort order
						var $last = jQuery("#"+$instance.attr("id")+" .zp-List h3:eq(0)");

						jQuery("#"+$instance.attr("id")+" .zp-List h3").each( function(i, e)
						{
							// If asc, place before the greater one
							if ( orderby == "asc" &&
									parseInt( jQuery(e).attr("rel") ) > parseInt(tempTitle) )
							{
								$last.before( "<h3 rel='"+orderedType+"'>"+tempTitle+"</h3>\n" );
								...
								return false;
									...
									parseInt( jQuery(e).attr("rel") ) < parseInt(tempTitle) )
							{
								$last.after( "<h3 rel='"+orderedType+"'>"+tempTitle+"</h3>\n" );
