jQuery(document).ready(function()
{
			...
			|| jQuery("#zp-Browse-Collections-Select").length > 0 )
	{
		...
		window.zpACPagination = function zpACPagination(is_new_query, do_append)
		{
			...
			if ( do_append
					|| is_new_query == true
					|| jQuery("#zpSearchResultsPaging").children().length == 0 )
			{
				...
				{
					if ( jQuery("#zpSearchResultsPagingContainer .title").length == 0 )
						jQuery("#zpSearchResultsPagingContainer").prepend("<span class='title'>Page</span>");

					if ( i == 1 )
						jQuery("#zpSearchResultsPaging").append("<a class='selected' href='javascript:void(0)'><span>"+i+"</span></a>");
					else
						jQuery("#zpSearchResultsPaging").append("<a href='javascript:void(0)'><span>"+i+"</span></a>");
				}
			}
		};
