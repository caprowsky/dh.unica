jQuery(document).ready(function()
{
	...
	var zpTopLevel = false; if ( jQuery("#ZP_TOPLEVEL").length > 0 )
	{
		zpTopLevel = jQuery("#ZP_TOPLEVEL").text();
	...
	function zp_get_collections ( request_start, request_last, update )
	{
		...
		jQuery.ajax(
		{
			...
			success: function(data)
			{
				...
				if ( zpCollectionId
						&& jQuery("#zp-Browse-Collections-Select option.toplevel").length == 0 )
					if ( jQuery("#ZP_COLLECTION_NAME").length > 0 )
						...
						&& zp_items.data.length > 0 )
				{
							...
							&& zp_items.meta.request_next != "false" )
					{
								...
								&& zp_items.meta.request_last > 0 )
						{
							...
							jQuery("#zpSearchResultsPaging").width( jQuery("#zpSearchResultsPaging a").length * 50 + "px" );

							// Add a scroller, if doesn't exist
							if ( jQuery("#zpSearchResultsPagingScroller").length == 0 )
							{
								// Update the width of the crop
								// Note: Based on five page numbers shown
								jQuery("#zpSearchResultsPagingCrop").width( jQuery("#zpSearchResultsPaging a.selected").outerWidth() * 5 );

								// Add the scroller
								jQuery("#zpSearchResultsPagingContainer").append( '<div id="zpSearchResultsPagingScroller"><span id="zpSearchResultsPagingBack">&#8249;</span><span id="zpSearchResultsPagingForward">&#8250;</span></div>' );

								// Add event handler for back
								jQuery("#zpSearchResultsPagingContainer").on( 'click', '#zpSearchResultsPagingBack', function()
								{
									var leftPos = parseInt( jQuery("#zpSearchResultsPaging").css('left') );
									var shiftW = parseInt(jQuery("#zpSearchResultsPaging a.selected").css('width')) + ( parseInt(jQuery("#zpSearchResultsPaging a.selected").css("border-left-width")) * 2 );

									// Don't go too far forward/right = past zpSearchResultsPaging.width
									if ( leftPos != 0 )
										jQuery("#zpSearchResultsPaging").css('left', leftPos+shiftW+'px');
								});

								// Add event handler for forward
								jQuery("#zpSearchResultsPagingContainer").on( 'click', '#zpSearchResultsPagingForward', function()
								{
									var leftPos = parseInt( jQuery("#zpSearchResultsPaging").css('left') );
									var shiftW = parseInt(jQuery("#zpSearchResultsPaging a.selected").css('width')) + ( parseInt(jQuery("#zpSearchResultsPaging a.selected").css("border-left-width")) * 2 );

									// Don't go too far back/left = past 0
									if ( ( leftPos * -1 ) < ( jQuery("#zpSearchResultsPaging").width() - 50 ) )
										jQuery("#zpSearchResultsPaging").css('left', leftPos-shiftW+'px');
								});
							}
						}
