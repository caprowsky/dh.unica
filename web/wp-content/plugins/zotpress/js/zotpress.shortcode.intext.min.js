...
request_last=0;jQuery.ajax({url:zpShortcodeAJAX.ajaxurl,ifModified:!0,data:{'action':'zpRetrieveViaShortcode','instance_id':$instance.attr("id"),'type':"intext",'item_key':params.zpItemkey,'style':params.zpStyle,'title':params.zpTitle,'showimage':params.zpShowImages,'showtags':params.zpShowTags,'downloadable':params.zpDownloadable,'shownotes':params.zpShowNotes,'showabstracts':params.zpShowAbstracts,'citeable':params.zpCiteable,'target':params.zpTarget,'urlwrap':params.zpURLWrap,'highlight':params.zpHighlight,'sortby':params.zpSortBy,'order':params.zpOrder,'update':update,'request_start':request_start,'request_last':request_last,'zpShortcode_nonce':zpShortcodeAJAX.zpShortcode_nonce},xhrFields:{withCredentials:!0},success:function(data)
{var zp_items=jQuery.parseJSON(data);if(zp_items.status=='error')
{console.log('Zotpress Error: '+zp_items.data);var hideErrMsg='';if(jQuery("#"+zp_items.instance+" .zp-List .zp-Entry").length>0)
hideErrMsg=' class="hide"';jQuery("#"+zp_items.instance+" .zp-List").removeClass('loading').append('<p'+hideErrMsg+'>Zotpress Error: '+zp_items.data+'</p>')}
else{if(typeof zp_items!='undefined'&&zp_items!=null&&parseInt(zp_items)!=0&&zp_items.data.length>0)
{if(params.zpShowNotes==!0)var tempNotes="";if(params.zpTitle==!0)var tempTitle="";var $postRef=jQuery($instance).parent();if(update===!1)
...
zp_format_intext_citations($instance,params.zpItemkey,zp_items.data,params,update);zp_format_intextbib($instance,zp_items,params.zpItemkey,params,update);if(update===!1)
{jQuery("#"+zp_items.instance+" .zp-SEO-Content .zp-Entry").unwrap()}
if(params.zpShowNotes==!0&&tempNotes.length>0)
...
else{var sortby=params.zpSortBy;var orderby=params.zpOrder;if(['author','date'].indexOf(sortby)!==-1&&jQuery("#"+zp_items.instance+" .zp-List .csl-left-margin").length==0)
{var sortOrder="zp-author-date";if(sortby=="date")sortOrder="zp-date-author";jQuery("#"+zp_items.instance+" .zp-List div.zp-Entry").sort(function(a,b)
{var an=jQuery(a).data(sortOrder).toLowerCase(),bn=jQuery(b).data(sortOrder).toLowerCase();if(an>bn)
if(orderby=="asc")
return 1;else return-1;else if(an<bn)
if(orderby=="asc")
return-1;else return 1;else return 0}).detach().appendTo("#"+zp_items.instance+" .zp-List")}
else{console.log("numbering resort? if no csl-left-margin")}}}}
...
tempItem_data[value.key]=value});item_data=tempItem_data;var intextGroupTracker={};jQuery.each(intext_citations,function(index,intext_citation)
{var intext_citation_output="";var $postRef=jQuery($instance).parent();var tempId=intext_citation.replace(/{/g,"-").replace(/}/g,"-").replace(/,/g,"_").replace(/:/g,"-");var intext_citation_id="zp-InText-zp-ID-"+tempId+"-wp"+jQuery(".ZP_POSTID",$instance).text();var intext_group_index=0;if(jQuery("."+intext_citation_id,$postRef).length>1)
if(!intextGroupTracker.hasOwnProperty(intext_citation_id))
intextGroupTracker[intext_citation_id]=0;else intext_group_index=intextGroupTracker[intext_citation_id];var intext_citation_params=JSON.parse(jQuery("."+intext_citation_id+":eq("+intext_group_index+")",$postRef).attr("rel").replace(/'/g,'"'));intext_citation_split=intext_citation.split("},{");intext_citation=new Array();jQuery.each(intext_citation_split,function(id,item)
{item_parts=item.split(":");item_pages=!1;if(intext_citation_params.pages!="np")
{item_pages=intext_citation_params.pages.split("--");if(item_pages[id]=="np")
item_pages[id]=!1;item_pages=item_pages[id]}
intext_citation[id]={"api_user_id":item_parts[0].replace("{",""),"key":item_parts[1].replace("}",""),"post_id":jQuery(".ZP_POSTID",$instance).text(),"pages":item_pages,}});var group_authors=[];jQuery.each(intext_citation,function(cindex,item)
...
{tempAuthorCount++;if(ai!=0&&tempAuthorCount>1)item_authors+=", ";if(author.hasOwnProperty("name"))item_authors+=author.name;else if(author.hasOwnProperty("lastName"))item_authors+=author.lastName});if(group_authors.indexOf(item_authors)==-1)
...
if(jQuery.isArray(item_authors)&&item_authors.length>1&&item_authors.indexOf("et al")==-1)
{var temp_and=" &amp; ";if(intext_citation_params.and=="and")
temp_and=" and ";else if(intext_citation_params.and=="comma-and")
temp_and=", and ";else if(intext_citation_params.and=="comma-amp")
temp_and=", &amp; ";else if(intext_citation_params.and=="comma")
temp_and=", ";else temp_and=" &amp; ";var temp=item_authors.join().replace(/,/g,", ");item_authors=temp.substring(0,temp.lastIndexOf(", "))+temp_and+item_authors[item_authors.length-1]}}
...
{var multip="p. ";if(item.pages.indexOf("-")!=-1)
multip="pp. ";item_citation=item_citation.replace("%p%",multip+item.pages)}
...
window.zpIntextCitations["post-"+item.post_id][item.key].intexttitle="";item_citation="<a "+window.zpIntextCitations["post-"+item.post_id][item.key].intexttitle+"class='zp-ZotpressInText' href='#zp-ID-"+item.post_id+"-"+item.api_user_id+"-"+item.key+"'>"+item_citation+"</a>";if(intext_citation_params.format.indexOf("sup")!="-1")
item_citation="<sup>"+item_citation+"</sup>";intext_citation[cindex].intext=item_citation});var intext_citation_pre="";if(intext_citation_params.brackets)intext_citation_pre="[";var intext_citation_post="";if(intext_citation_params.brackets)intext_citation_post="]";intext_citation_output=intext_citation_pre;jQuery.each(intext_citation,function(cindex,item)
{if(cindex!=0)
{if(intext_citation_params.separator=="comma")
intext_citation_output+=", ";else intext_citation_output+="; "}
intext_citation_output+=item.intext});intext_citation_output+=intext_citation_post;jQuery("."+intext_citation_id+":eq("+intext_group_index+")",$postRef).removeClass("loading").html(intext_citation_output);if(intextGroupTracker.hasOwnProperty(intext_citation_id))
intextGroupTracker[intext_citation_id]++})}
function zp_format_intextbib($instance,zp_items,zp_itemkeys,params,update)
{var tempHasNum=!1;var zpPostID=jQuery(".ZP_POSTID",$instance).text();var itemNumOrderArr=[];jQuery.each(zp_items.data,function(index,item)
{var tempItem="";var $item_ref=jQuery("#"+zp_items.instance+" .zp-List #zp-ID-"+zpPostID+"-"+item.library.id+"-"+item.key);var tempItemYear="0000";var tempItemDate="0000";if(item.meta.hasOwnProperty('parsedDate'))
tempItemYear=item.meta.parsedDate.substring(0,4);if(item.data.hasOwnProperty('date'))
tempItemDate=item.data.date;var tempAuthor=item.data.title;if(item.meta.hasOwnProperty('creatorSummary'))
tempAuthor=item.meta.creatorSummary.replace(/ /g,"-");if(params.zpTitle==!0&&tempTitle!=tempItemYear)
{tempTitle=tempItemYear;tempItem+="<h3>"+tempTitle+"</h3>\n"}
tempItem+="<div id='zp-ID-"+jQuery(".ZP_POSTID",$instance).text()+"-"+item.library.id+"-"+item.key+"'";tempItem+=" data-zp-author-date='"+tempAuthor+"-"+tempItemDate+"'";tempItem+=" data-zp-date-author='"+tempItemDate+"-"+tempAuthor+"'";tempItem+=" class='zp-Entry zpSearchResultsItem zp-Num-"+window.zpIntextCitations["post-"+zpPostID][item.key].num;if(update===!0)tempItem+=" zp_updated";if(jQuery("#"+zp_items.instance+" .ZP_SHOWIMAGE").text().trim().length>0&&item.hasOwnProperty('image'))
...
var temp=zp_forcenumbers(zp_items.instance,item,zpPostID);item=temp.item;itemNumOrderArr[temp.itemNumOrder.order]=temp.itemNumOrder.key;if(/csl-left-margin/i.test(item.bib))
tempHasNum=!0;tempItem+=item.bib;if(params.zpShowAbstracts==!0&&(item.data.hasOwnProperty('abstractNote')&&item.data.abstractNote.length>0))
...
else{if($item_ref.length==0)
{jQuery("#"+zp_items.instance+" .zp-List").append(tempItem)}
else{$item_ref.replaceWith(jQuery(tempItem))}}});if(tempHasNum&&update===!1)
{if(jQuery("#"+zp_items.instance+" .zp-List .zp-Entry").length==0)
{console.log("no entries, hasnum, not updating")}
else{for(var num=1;num<itemNumOrderArr.length;num++)
{var item=window.zpIntextCitations["post-"+zpPostID][itemNumOrderArr[num]];var $item=jQuery("#"+"zp-ID-"+zpPostID+"-"+window.zpIntextCitations["post-"+zpPostID][itemNumOrderArr[num]].api_user_id+"-"+itemNumOrderArr[num]);jQuery("#"+zp_items.instance+" .zp-List").append($item)}}}
else{console.log("not hasnum or updating")}}
function zp_forcenumbers(zp_instance,zp_item,zpPostID)
{var itemNumOrder={'order':0,'item_key':''};if(/csl-left-margin/i.test(zp_item.bib))
{var $item_content=jQuery.parseHTML(zp_item.bib);var item_num_content=jQuery(".csl-left-margin",$item_content).text();item_num_content=item_num_content.replace(item_num_content.match(/\d+/)[0],window.zpIntextCitations["post-"+zpPostID][zp_item.key].num);jQuery(".csl-left-margin",$item_content).text(item_num_content);zp_item.bib=jQuery('<div>').append($item_content).html();itemNumOrder.order=window.zpIntextCitations["post-"+zpPostID][zp_item.key].num;itemNumOrder.key=zp_item.key}
else{if(jQuery("#"+zp_instance+" .ZP_FORCENUM").text().length>0&&jQuery("#"+zp_instance+" .ZP_FORCENUM").text()=="1")
